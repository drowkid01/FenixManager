...
packets_to_install=(apt-transport-https python3 python3-pip neovim htop fail2ban sqlite3 debsums zip unzip mlocate ufw net-tools jq git)
updates_command=(update full-upgrade autoremove)
pip_packages=(colorama argparse requests)

if [ $(id -u) -ne 0 ]; then
    error "Este SCRIPT debe ser ejecutado como root: sudo ./$script_name"
    exit 1
fi
        ...
        banner="${RED}
           ...
           |    --\//--    |
            \_  \     /  _/
              \__  |  __/
                 \ _ /
                _/   \_  ${YELLOW} @drowkid01 ${RED}
               / _/|\_ \ ${YELLOW} Fenix ADMRPlus${RED}
               /   |  \   ${YELLOW}Version: ${GREEN}${version}${RED}
                 / v \ 
"
        echo -e "$banner"
        echo -e "${WHITE}Para mostrar el panel de administración, ejecutar el siguiente comando : ${GREEN}fenix ${WHITE}"
        }
        print_fenix_banner
'
